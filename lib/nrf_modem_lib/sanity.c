...
BUILD_ASSERT(DT_REG_ADDR(DT_NODELABEL(cpuapp_cpucell_ipc_shm_ctrl)) % 4 == 0,
	"libmodem CTRL region base address must be word aligned");
BUILD_ASSERT(DT_REG_ADDR(DT_NODELABEL(cpuapp_cpucell_ipc_shm_heap)) % 4 == 0,
	"libmodem TX region base address must be word aligned");
BUILD_ASSERT(DT_REG_ADDR(DT_NODELABEL(cpucell_cpuapp_ipc_shm_heap)) % 4 == 0,
	"libmodem RX region base address must be word aligned");
